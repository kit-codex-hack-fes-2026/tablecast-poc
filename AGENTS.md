...
- D1の読み書きとfixtureは既存schemaを使ったDrizzleに統一する。DB変更では[API skillの設計・検証手順](.agents/skills/tablecast-api/SKILL.md#db変更の設計と検証)に従い、SQL断片を残す根拠と、呼出し経路全体のDB往復数を確認する。`env.TABLECAST_DB.prepare()`などの直接操作を新設しない。複数操作は`db.batch()`を使う。生SQLはmigration・PRAGMA・query builderで表現できないクエリに限定する。
