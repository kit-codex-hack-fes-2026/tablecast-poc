...
test(
  ...
  { tag: ["@product", "@tablecast"] },
  async ({ page }) => {
    const directory = process.env.TABLECAST_VIDEO_DIR ?? "dist/product";
    ...
    const timing = { scenes: projectSchema.shape.scenes.parse(input.scenes) };
    const focus = timing.scenes
      .find((scene) => scene.id === "readback")
      ?.pointFocus?.find((point) => point.point === 1);
    if (!focus) throw new Error("次の行への強調時刻がありません");
    const bounds = await page.evaluate((at) => {
      ...
      timeline.invalidate().seek(Number(scene?.dataset.start) + at + 0.5);
