...
const guest = sample.scenes.find(
  (scene) => scene.role === "customer" && scene.media?.project,
)?.media;
if (!guest?.project) throw new Error("客側の撮影projectがありません");
...
const scenes = sample.scenes.filter((scene) => scene.media?.file === guest.file);
...
await writeFile(projectPath(), JSON.stringify(sample, null, 2));
