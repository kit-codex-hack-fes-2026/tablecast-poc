    ...
    for (const target of f.targets) {
      ...
      if (board.connections.some((c) => c.id === target)) {
        animations.push(
          ...
          `tl.fromTo("${selector} .tech-trace", {strokeDashoffset:1}, {strokeDashoffset:0,duration:0.65,ease:"power2.inOut",immediateRender:false}, ${at});`,
        );
      } else {
        animations.push(
          `tl.fromTo("${selector} .tech-outline", {autoAlpha:0}, {autoAlpha:1,duration:0.2,immediateRender:false}, ${at});`,
        );
      }
    }
  });
