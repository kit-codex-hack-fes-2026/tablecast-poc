...
v13では、下記で再撮影した管理画面を全編へ組み込んだ。客・店員側は成功済み収録を使い、[ポインター](POINTER.md)を修正して全編を再出力する。

Issue #1の動画生成基盤として、操作の完了ではなく「見せたい情報が撮れたこと」を編集の前提にする。撮影対象の正本は `capture-plan.json`。端末と話者、発話、採用済み素材は `sample.json`。
...
6. `build` でも採用可能なテイク、撮影定義の一致、表示時間、実測位置・時刻と編集の一致を検査する。撮影条件を変更したのに古い素材を新定義として採用することはできない。`capture-plan.json` の変更だけでは旧完成版を壊さず、再撮影成功後に `sample.json` の `capture` と素材を更新する。
...
客側の再撮影では、最初は承認結果より先に履歴を開いてしまう撮影順の問題、その後は空の音声入力と `VOICE_INTERNAL_ERROR` を検出した。履歴を開く条件と音声中の操作順を修正し、失敗したテイクは不採用。注文成立までの新しい客側一式は未完成。現在の商品紹介は成功済みの客・店員素材と再撮影した管理画面を組み込んだv13。保存先・残件は [HANDOFF.md](HANDOFF.md) を参照する。
