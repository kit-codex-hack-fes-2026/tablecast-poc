...
import { captureShot, bindShotZoom, type ShotEvidence } from "../../scripts/tablecast-shot.ts";
import { zoomRegions } from "../../scripts/tablecast-zoom.ts";
import { readProject, readCaptureProject, projectPath } from "../../scripts/tablecast-project.ts";
import { composeCursor } from "../../scripts/tablecast-cursor.ts";
...
try {
  ...
  const target =
    role === "admin"
      ...
      : page.getByRole("tab", { name: "注文の受付", exact: true });
  ...
  try {
    ...
    await sleep(2200);
    if (role === "staff") {
      const accept = page.getByRole("button", { name: "調理を開始", exact: true });
