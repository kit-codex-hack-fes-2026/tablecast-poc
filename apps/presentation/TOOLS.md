...
- `GENERATION.md`：工程と採用判断の正本。`HANDOFF.md`：商品紹介v14・技術紹介v17の保存状態、残件、再開手順。
- `frame.md`：映像設計と評価状態。動画の仕上げは一旦区切り、技術紹介の後半は継続改善の対象。技術検証をデザインの承認と混同しない。
- `projects/tablecast-main-rerecord.json` / `styles.css` / `scripts`：現在動作する動画の正本。実装済みであることと、デザインが承認済みであることを混同しない。
