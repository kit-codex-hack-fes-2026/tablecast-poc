...
for (const { language, locale, labels, nextLabels, nextLanguage } of [
  ...
  test(`${language === "English" ? "英語" : language}の来店履歴を読み取り、古いログと注文原文を保ち店舗切替で混在させない`, async ({
    ...
    const target = initial.sessions.find((session) => session.id === `${storeId}-table-11-session`);
    ...
    await expect(otherHistoryLink).toHaveAttribute("href", `/admin/stores/${otherStoreId}/visits`);
