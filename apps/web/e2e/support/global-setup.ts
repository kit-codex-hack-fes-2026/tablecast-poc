...
export default async function setup() {
  ...
  await writeFile(
    seedConfig,
    JSON.stringify({ ...base, name: "tablecast-e2e-seed", vars, d1_databases: d1, r2_buckets: r2 }),
  );
  await execute(
    "node",
    [
  ...
  try {
    ...
    start("docker", [
      ...
      "--rm",
      "--name",
      `tablecast-e2e-mailpit-${runtime.ports.mailpit}`,
      "-p",
    ...
    while (Date.now() < deadline) {
      try {
        const results = await Promise.all([
          fetch(`${runtime.origin}/api/admin/stores`),
          fetch(`${vars.TABLECAST_MAILPIT_URL}/api/v1/info`),
        ]);
        if (results[0]?.status === 401 && results[1]?.ok) return stop;
      } catch {}
