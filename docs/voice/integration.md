...
5. Agents APIの `final_answer` の本文差分をSSEでWebへ渡し、Webが同じdelegation IDの `session.commentary.append` へ上限内で順次渡す。Agentsの進捗用 `commentary` は渡さない。GPT-Liveが結果を自然に説明する。全文生成・読み終わりを待ってからまとめて渡さない。
