...
| 確認                          | 結果・証拠                                                                                                                                                                  |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bun --no-env-file run check` | 全体format/lint/typecheck成功。ルート7件、Python66件、Web音声単体6件、API61件成功                                                                                           |
| 追加のAPI修正                 | 実workerd・D1・DOでWebSocket終了・再接続、ラストオーダー対象範囲、設定公開の競合・rollback・卓への通知境界を検証                                                            |
| 更新後のUI部品                | Storybook22件、Web音声単体6件成功。日英・選択・エラー・アクセシビリティ                                                                                                     |
| ブラウザー                    | 最新版の本番相当環境で22件成功。日英注文・提供・会計・OAuth同意、通知切断と復旧、確認期限、商品編集・明示公開、設定公開反映、管理画面の通知復旧、文字拡大・キーボードを検証 |
...
ローカルでは安全情報・選択肢・プラン・キャストなどの手動編集と、APIが返す固定日本語の検証理由の英語化が残る。今回の8項目の商品編集を、全設定の編集完了とは扱わない。
