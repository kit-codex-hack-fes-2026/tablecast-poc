...
Honoの`app.ts`は認証・共通middlewareと公開入口を組み立て、卓と店舗のルートは`src/routes`で入力検証を行う。業務更新は`modules/operations.ts`、読取投影は`modules/table-state.ts`、設定更新は`modules/configuration.ts`が所有する。DrizzleのD1 adapterとSQL templateを使い、条件付き更新・監査イベントの同一batchと店舗境界を維持する。単にDB処理を転送するrepository層は作らない。
