...
Googleには氏名・店舗名・権限（owner/admin/member）を標準seedのnameへ渡す。選択画面はemulateの標準表示を使うためメールが主行となり、写真表示・主副行の並べ替え・既定Test Userの非表示は行わない。pictureは標準のOAuthプロフィール情報として渡し、店舗管理画面では生成画像を使用する。表示上の権限は案内であり、実際の認可はログイン後のDB所属による。
...
独自の管理者メールをローカル資格情報へ指定した場合は、そのアカウントを追加のオーナーとして保持し、名簿のGoogleオーナーも同じ店舗へ所属させる。標準の3人/orgへ独自アカウントが追加されるため、この場合だけ所属人数が増える。
