...
管理画面と卓上画面は50件ごとに続きを取得する。`list_games`は50件ごとの一覧、`get_game`は最新20版と公開状態、`get_game_source`は指定版のソースを返す。修正は新しい版として登録する。MCPに公開・停止ツールは与えない。公開・停止・以前の版の再公開は店舗管理者の画面操作で行う。
