...
既存の閉卓履歴indexは`closed_at`順であり、利用中を含む不変cursorには対応しない。migration `0015_tablecast_floor_timeline.sql`で実卓用の`(store_id, opened_at DESC, id DESC) WHERE kind='table'`を追加する。卓をjoinせず6列だけを取得し、期間queryは1文・1往復とする。店舗認可を含むHTTP経路は4往復以内を試験する。

現行の単一selectとindexで、100卓・100／10,000履歴・同一日の30件取得を各3標本検証する。返却内容とcursor、往復数、binding時間、HTTP時間、JSON bytesを同じrequestで測る。性能予算は4往復以内・1,000ms未満・10,000 bytes未満であり、件数増加時にも往復数を変えない。大きな二言語snapshotや会計の追加取得はない。

open／closedを別queryやset operationへ分ける案は採用しない。単一selectで現在の予算を満たし、2つのcursorの統合処理を増やす理由がないためである。ただし、このindexは区間の両端を同時に最適化しない。長期滞在や該当なしの日は読取対象が増える余地がある。remote D1のSQL実行時間・読取行数・往復遅延、本番規模の分布は未測定であり、ローカルの時間をそのまま本番保証にしない。

SQL断片は新設しない。採用済みDrizzleの`select`、条件、並び順、`limit`で表現し、検証fixtureも既存schemaとDrizzleを使う。schemaの境界を増やさず、読取処理は既存の`history.ts`に置く。
...
| 負荷             | 100卓・100／10,000履歴を各3標本。HTTP経路の往復・binding時間・応答時間・bytes・30件の内容とcursorを測定                                              |
