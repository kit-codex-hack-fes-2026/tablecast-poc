...
音声はGPT-Live 1の公式LiveKitプラグインを使い、client delegationを既存Mastraへ接続する。既定ペルソナは親しみと丁寧さ、愛嬌を持つ若い成人女性の飲食店スタッフとする。
...
| [実現可能性と技術課題](feasibility.md) | Hono・Mastra・LiveKit・GPT-Liveの接続、安全性、改善  |
...
| ローカルでもExpressive Modeを前提 | 公式GPT-Live plugin＋Mastraへのclient delegation |
| 汎用スターターの多数の層・skills  | 少数の目的別ファイルと4つの短いskill             |

## 同梱範囲

仕様、Web/API/Python実装、lockfile、migration、デモ画像、開発スクリプト、CI定義を含む。DB・秘密情報・node_modules・仮想環境はGit管理しない。旧Inworld patchとbundleは過去の検証資料として保持し、通常音声の実行時依存には含めない。
