...
- 新規実装もroute・認可・service・queryを通した取得経路を見る。必要なID、データの寿命、前の結果への依存を整理し、取得済みの行を渡せる箇所や重複取得を確認する。経路全体のDB往復を数え、ループ内・卓数に比例する問合せと不要な直列awaitを除く。独立した読取は`db.batch()`、親子取得はjoin・subqueryを使う。D1の`Promise.all`は往復数の削減にはならない。
...
- 取得の統合・再読込の削除でも店舗・卓の認可条件を保つ。書込の版条件、batchの原子性、`mutation_id`による更新成立確認を維持し、D1非対応のtransactionを追加しない。価格・権限判定に使った行と版が条件付き更新の対象・版と一致することを確認する。snapshotとcursorの取得順を守り、取得中の更新をイベントで回復する。
- batchやjoinへの変更では同名列のalias、LEFT JOINの不成立、NULL、Dateと公開timestampの単位を確認する。日付をSQLへ埋め込んでschemaの変換を迂回しない。集約をJSへ移して取得行数を増やさず、既存index・ページングを維持する。問合せ数の固定テストを追加するだけで設計確認を代替しない。
