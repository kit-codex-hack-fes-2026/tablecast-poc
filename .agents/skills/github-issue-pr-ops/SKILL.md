...
最新headのチェック・レビュー・マージ可否を確認し、未実行・待機中・過去の別commitでの成功を区別する。検証結果はPRへ集約し、通常のCI待ちごとにIssueへ同じ報告を追加しない。配備済みのStorybook・React Emailのpreview URLと変更したコンポーネントの直リンクは[プレビューの記載手順](references/pr-and-merge.md#コンポーネントプレビューのリンク)に従ってPR本文へ載せる。
